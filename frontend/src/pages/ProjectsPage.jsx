    ...
    <div className="min-h-[calc(100vh-4rem)] py-10 px-4 sm:px-6 lg:px-8 bg-slate-50">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center justify-between">
          ...
          {/* Status Filter Tabs */}
          <div className="flex items-center gap-1 overflow-x-auto pb-2 md:pb-0 scrollbar-none bg-white p-1 rounded-2xl border border-slate-200 shadow-2xs">
        ...
        {/* Error Alert */}
        {error && (
          <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 flex items-start justify-between gap-4">
            <div className="flex items-start gap-3">
              <AlertCircle className="w-5 h-5 text-rose-600 shrink-0 mt-0.5" />
              <div>
                <h4 className="text-sm font-semibold text-rose-700">Unable to Load Projects</h4>
                <p className="text-xs text-rose-600 mt-1">{error}</p>
              </div>
            </div>
            <button
              onClick={fetchProjects}
              className="px-3 py-1 rounded-lg bg-rose-600 hover:bg-rose-700 text-white text-xs font-medium cursor-pointer"
            >
              Retry
            </button>
          </div>
        ...
        {loading && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="bg-white rounded-2xl p-6 border border-slate-200 animate-pulse space-y-4 shadow-xs"
              >
        ...
        {!loading && !error && projects.length > 0 && filteredProjects.length === 0 && (
          <div className="bg-white rounded-2xl p-10 border border-slate-200 text-center space-y-3 shadow-xs">
            ...
            <h3 className="text-sm font-semibold text-slate-800">No Projects Match Your Filter</h3>
            ...
            <button
              ...
              className="px-3.5 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium cursor-pointer"
            >
              Reset Filters
            </button>
          </div>

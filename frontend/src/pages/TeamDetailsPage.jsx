...
export default function TeamDetailsPage() {
  ...
  const loadWorkspaceInvites = useCallback(async () => {
    if (!id) return;
    try {
      const res = await invitationService.getWorkspaceInvitations(id);
      setWorkspaceInvites(res.data?.invitations || []);
    } catch (e) {
      // Non-leaders or unauthorized users won't fetch invitations
    }
  }, [id]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {/* Workspace Invitations Section (Leader Only) */}
      {isLeader && workspaceInvites.length > 0 && (
      ...
      {/* MODAL 0: Invite Team Member Modal (Leader only) */}
      {showInviteModal && (
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    Invite a user to join <strong>{team?.name}</strong>
                  </p>
                </div>
              </div>
              <button
              ...
              <form onSubmit={handleSendInvitation} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-1.5">
                    Generates a secure token valid for 7 days. Invited users join as workspace members.
                  </p>
                </div>
